    ...
    Variants {
        ...
        LazyLoader {
            ...
            component: PanelWindow { // Bar window
                ...
                property list<HyprlandWorkspace> workspacesForScreen: Hyprland.workspaces.values.filter(workspace => workspace.monitor && workspace.monitor.name == screen.name)
                property var activeWorkspaceWithFullscreen: workspacesForScreen.filter(workspace => ((workspace.toplevels.values.filter(window => window.wayland?.fullscreen)[0] != undefined) && workspace.active))[0]

                property bool autoHideEnable: activeWorkspaceWithFullscreen || Config?.options.bar.autoHide.enable
